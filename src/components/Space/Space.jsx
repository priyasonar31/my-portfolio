...
import spacemanScene from "../../assets/3d.glb";
...
const Spaceman = ({ scale, position }) => {
  ...
  console.log("spaceman -->", position, scale);
...
const SpacemanCanvas = ({ scrollContainer }) => {
  ...
  const [scale, setScale] = useState([12, 12, 12]);
  ...
  return (
    <Canvas className={`w-full h-screen bg-transparent z-10`} camera={{ near: 0.1, far: 1000 }}>
      <Suspense>
        ...
        <Spaceman rotationX={rotationX} rotationY={rotationY} scale={[2,2,2]} position={[0.2, -0.4, 0]} />
      </Suspense>
    </Canvas>
  );
};
